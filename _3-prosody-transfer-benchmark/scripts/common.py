#!/usr/bin/env python3
"""Shared utilities for the prosody transfer benchmark."""
...
PRIMARY_METRIC = "f0_mean"
METRICS = ("f0_mean", "f0_std", "f0_range", "speaking_rate")
REQUIRED_COLUMNS = {
    "sample_id", "pair_id", "text", "condition",
    "reference_path", "output_path", "model", "seed", "notes",
}
VALID_CONDITIONS = {"breathy", "modal"}
...
def validate_metadata(rows: list[dict[str, str]]) -> None:
    ...
    incomplete = [pair_id for pair_id, conditions in pair_conditions.items()
                  if conditions != VALID_CONDITIONS]
    if incomplete:
        raise ValueError(
            "each pair_id must contain both breathy and modal references; incomplete: "
