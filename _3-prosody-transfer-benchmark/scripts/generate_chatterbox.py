#!/usr/bin/env python3
"""Generate Chatterbox TTS outputs for prosody transfer benchmark.
...
def get_unique_outputs(rows: list[dict]) -> list[tuple[str, str, str, str, str]]:
    ...
    unique = []
    for row in rows:
        if row["model"].strip().lower() != "chatterbox":
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Chatterbox TTS outputs for prosody benchmark")
    parser.add_argument("--metadata", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--exaggeration", type=float, default=0.5,
                        help="Exaggeration scalar (0.0-1.0)")
    ...
    try:
        ...
        print("ERROR: chatterbox not installed. Run: pip install chatterbox-tts",
    ...
    for sample_id, pair_id, condition, ref_path, out_path in outputs:
        ...
        try:
            wav = model.generate(text=text, audio_prompt_path=str(ref_full),
                                 exaggeration=args.exaggeration)

            out_full = args.output_dir / Path(out_path).name
            out_full.parent.mkdir(parents=True, exist_ok=True)
            torchaudio.save(str(out_full), wav.cpu(), model.sr)
